...
```
testing/                              # Consolidated testing directory
...
- **Allure.Reqnroll**: Allure integration for Reqnroll (mandatory)
- **Allure.NUnit**: Allure integration for NUnit (mandatory)
...
### 2. Allure Integration Configuration

**Allure Configuration (allureConfig.json):**
```json
{
  "allure": {
    "directory": "allure-results",
    "links": [
      "https://arcteryx.atlassian.net/browse/{issue}",
      "https://github.com/arcteryx-ocp/ibs-api/issues/{issue}"
    ]
  }
}
```
...
- `ALLURE_RESULTS_DIRECTORY`: Directory for Allure results
- `ASPNETCORE_ENVIRONMENT`: Testing environment
- `TEST_ENVIRONMENT`: Target environment (local, preprod, prod)

### 3. Feature File Structure with Allure Labels

**Gherkin Syntax Requirements:**
```gherkin
# Features/{FeatureName}.feature
@invoice-management @allure.label.epic:Invoice_Management @allure.label.owner:IBS_Team
Feature: Invoice Retrieval by Order Number
  As a SFSC integration system
  I want to retrieve invoice details by order number
  So that I can process customer inquiries efficiently

  Background:
    Given the invoice service is available
    And test data is prepared

  @happy-path @regression @allure.label.severity:critical @allure.label.story:Successful_Invoice_Retrieval
  Scenario: Retrieve existing invoice by valid order number
    Given an invoice exists with order number "0123456789"
    When I request the invoice for order number "0123456789"
    Then I should receive the invoice details
    And the response should contain valid invoice data
  ...
  Scenario: Attempt to retrieve non-existent invoice
    Given no invoice exists with order number "0999999999"
...
**Allure Label Guidelines:**
- `@allure.label.epic:Epic_Name`: High-level feature grouping
- `@allure.label.story:Story_Name`: User story or feature
- `@allure.label.severity:level`: critical, major, normal, minor
- `@allure.label.owner:Team_Name`: Responsible team
- `@allure.label.testType:Type`: Positive, Negative, Regression

### 4. Step Definition Implementation with Allure

**Step Definition Standards:**
```csharp
[Binding]
[AllureFeature("Invoice Management")]
[AllureStory("Invoice Retrieval by Order Number")]
public class InvoiceRetrievalSteps
{
    ...
    [Given(@"an invoice exists with order number ""(.*)""")]
    [AllureStep("Verify invoice exists with order number: {orderNumber}")]
    public async Task GivenAnInvoiceExistsWithOrderNumber(string orderNumber)
    {
        // **Feature: invoice-management, Scenario: Retrieve invoice by order number**
        // **Validates: Requirements 1.1**
        
        AllureApi.AddParameter("orderNumber", orderNumber);
        ...
        AllureApi.AddAttachment("Expected Invoice", $"Order Number: {orderNumber}");
    }

    [When(@"I request the invoice for order number ""(.*)""")]
    [AllureStep("Request invoice for order number: {orderNumber}")]
    public async Task WhenIRequestTheInvoiceForOrderNumber(string orderNumber)
    {
        AllureApi.AddParameter("orderNumber", orderNumber);
        AllureApi.AddParameter("endpoint", $"/api/v1/orders/{orderNumber}/invoices");
        
        var response = await _driver.GetInvoiceByOrderNumberAsync(orderNumber);
        _scenarioContext["ApiResponse"] = response;
        
        AllureApi.AddAttachment("API Response", response.ToString());
    }

    [Then(@"I should receive the invoice details")]
    [AllureStep("Verify invoice details are returned successfully")]
    public void ThenIShouldReceiveTheInvoiceDetails()
    {
        var response = _scenarioContext.Get<ApiResponse<InvoiceModel>>("ApiResponse");
        var expectedInvoice = _scenarioContext.Get<object>("ExpectedInvoice");
        
        response.Success.ShouldBeTrue();
        response.Data.ShouldNotBeNull();
        response.CorrelationId.ShouldNotBeNullOrEmpty();
        
        AllureApi.AddParameter("responseSuccess", response.Success.ToString());
        AllureApi.AddParameter("correlationId", response.CorrelationId);
        AllureApi.AddAttachment("Invoice Details", $"Invoice Number: {response.Data?.InvoiceNumber}");
    }
}
```

**Allure Step Annotations:**
- `[AllureStep("Description")]`: Describes the step action
- `AllureApi.AddParameter()`: Adds test parameters
- `AllureApi.AddAttachment()`: Adds files or text attachments
- `AllureApi.AddLabel()`: Adds metadata labels
...
- **Testing Patterns**: [xUnit Test Patterns](http://xunitpatterns.com/)
