...
public sealed class OrderingFunctionalTestModeAttribute : BeforeAfterTestAttribute, ITraitAttribute
{
    public OrderingFunctionalTestModeAttribute(OrderingFunctionalTestMode mode)
    {
        Mode = mode;
    }

    public OrderingFunctionalTestMode Mode { get; }
    ...
    public static OrderingFunctionalTestMode Resolve(Type testClass, MethodInfo method)
    {
        var methodAttribute = method.GetCustomAttribute<OrderingFunctionalTestModeAttribute>(inherit: true);
        if (methodAttribute is not null)
        {
            return methodAttribute.Mode;
        }

        var classAttribute = testClass.GetCustomAttribute<OrderingFunctionalTestModeAttribute>(inherit: true);
        if (classAttribute is not null)
        {
            return classAttribute.Mode;
        }

        return OrderingFunctionalTestMode.Aspire;
    }

    public static OrderingFunctionalTestMode Resolve(Type testClass, string methodName, BindingFlags bindingFlags = BindingFlags.Instance | BindingFlags.Public)
    {
