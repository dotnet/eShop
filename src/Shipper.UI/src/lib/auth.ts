...
export const oidcConfig = {
  ...
  client_id: 'shipper-ui',
  redirect_uri: `${window.location.origin}/callback`,
  post_logout_redirect_uri: `${window.location.origin}/`,
  response_type: 'code',
  scope: 'openid profile roles shipping offline_access',
...
export const loginWithCredentials = async (
  ...
  try {
    ...
    localStorage.setItem('shipper_user', JSON.stringify({
      access_token: tokenResponse.access_token,
      token_type: tokenResponse.token_type,
      expires_in: tokenResponse.expires_in,
...
export const getStoredUser = (): User | null => {
  const stored = localStorage.getItem('shipper_user');
...
export const isShipper = (user: User | null): boolean => {
