...
public class VolumeDiscountStrategy : IDiscountStrategy
{
    ...
    {
        // Check if minimum quantity threshold is met
        var totalQuantity = context.Items.Sum(item => item.Units);

        if (!promotion.MinimumQuantity.HasValue || totalQuantity < promotion.MinimumQuantity.Value)
        {
            return 0;
        }
