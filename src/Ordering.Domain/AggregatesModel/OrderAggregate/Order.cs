...
#pragma warning restore CS0414
    ...
    public void SetReturnRequestedStatus()
    {
        if (OrderStatus != OrderStatus.Shipped)
        {
            throw new OrderingDomainException($"Cannot request return for order with status {OrderStatus}");
        }

        OrderStatus = OrderStatus.AwaitingReturn;
        Description = "The return has been requested.";
        AddDomainEvent(new OrderReturnRequestedDomainEvent(this));
    }

    public void SetStatusToRefunded()
    {
        // Implementation to set the order status to refunded
        OrderStatus = OrderStatus.Refunded;
        AddDomainEvent(new OrderRefundedDomainEvent(this));
    }

    public void SetReceivedReturnStatus()
    {
        if (OrderStatus != OrderStatus.AwaitingReturn)
        {
            throw new OrderingDomainException($"Cannot mark order with status {OrderStatus} as received return.");
        }

        OrderStatus = OrderStatus.RecievedReturn;
        Description = "The return has been received.";
        AddDomainEvent(new OrderReceivedReturnDomainEvent(this));
    }
    ...
    private void StatusChangeException(OrderStatus orderStatusToChange)
    {
        throw new OrderingDomainException(
            $"Is not possible to change the order status from {OrderStatus} to {orderStatusToChange}. Please request a return.");
    }
